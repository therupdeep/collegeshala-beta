function checkout(amount,credits,authToken) {
	...
	var options = {
		...
		"handler": function (response){
			...
			completePurchase(response.razorpay_payment_id,amount,credits,authToken);
...
function completePurchase(paymentid,amount,credits,authToken) {
	$.ajax({
		...
		error: function error(jqXHR,textStatus,errorThrown) {
			console.error('Error: ', textStatus, ', Details: ', errorThrown,'stack:',errorThrown.stack);
			console.error('Response: ', jqXHR.responseText);
			alert('An error occured. Please contact collegeshala for refund against payment id:',paymentid);
		}
	});
}
